...
$cases = @(
    ...
    [pscustomobject]@{ Name = "anndata h5ad container"; Prompt = "用 AnnData 读写 h5ad，管理 obs/var 元数据和 backed mode 稀疏矩阵"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "anndata" },
    [pscustomobject]@{ Name = "scvi latent model"; Prompt = "用 scVI 和 scANVI 做 single-cell batch correction、latent model 和 cell type annotation"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "scvi-tools" },
    [pscustomobject]@{ Name = "deeptools ngs tracks"; Prompt = "用 deepTools 把 BAM 转 bigWig，并围绕 TSS 画 ChIP-seq heatmap profile"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "deeptools" },
    [pscustomobject]@{ Name = "bioservices cross database"; Prompt = "用 BioServices 同时查询 UniProt、KEGG、Reactome 并做 ID mapping"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "bioservices" },
    [pscustomobject]@{ Name = "alphafold predicted structure"; Prompt = "从 AlphaFold Database 按 UniProt ID 下载 mmCIF，并检查 pLDDT 和 PAE"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "alphafold-database" },
    [pscustomobject]@{ Name = "clinvar clinical significance"; Prompt = "查询 ClinVar 中 BRCA1 variant 的 clinical significance、VUS 和 review stars"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "clinvar-database" },
    [pscustomobject]@{ Name = "cosmic cancer mutation"; Prompt = "查询 COSMIC cancer mutation、Cancer Gene Census 和 mutational signatures"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "cosmic-database" },
    [pscustomobject]@{ Name = "ensembl vep orthologs"; Prompt = "用 Ensembl REST 查询 gene、orthologs、VEP variant effect 和坐标转换"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "ensembl-database" },
    [pscustomobject]@{ Name = "ncbi gene metadata"; Prompt = "用 NCBI Gene 查询 TP53 symbol、RefSeq、GO annotation 和基因位置"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "gene-database" },
    [pscustomobject]@{ Name = "gwas catalog traits"; Prompt = "查询 GWAS Catalog 中 rs ID、trait association、p-value 和 summary statistics"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "gwas-database" },
    [pscustomobject]@{ Name = "kegg pathway mapping"; Prompt = "用 KEGG REST 做 pathway mapping、ID conversion 和 metabolic pathway 查询"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "kegg-database" },
    [pscustomobject]@{ Name = "open targets evidence"; Prompt = "用 Open Targets 查询 target-disease association、tractability、safety 和 known drugs"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "opentargets-database" },
    [pscustomobject]@{ Name = "rcsb pdb structure"; Prompt = "在 RCSB PDB 按 sequence similarity 搜索结构并下载 PDB/mmCIF 坐标"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "pdb-database" },
    [pscustomobject]@{ Name = "reactome enrichment"; Prompt = "用 Reactome 做 pathway enrichment、gene-pathway mapping 和 disease pathway 分析"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "reactome-database" },
    [pscustomobject]@{ Name = "string ppi network"; Prompt = "用 STRING API 查询 protein-protein interaction network、GO enrichment 和 hub proteins"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "string-database" },
    [pscustomobject]@{ Name = "cellxgene census"; Prompt = "查询 CZ CELLxGENE Census 的 human lung epithelial cells expression data 和 metadata"; Grade = "M"; TaskType = "research"; ExpectedPack = "bio-science"; ExpectedSkill = "cellxgene-census" },
    ...
    [pscustomobject]@{ Name = "medical histolab wsi"; Prompt = "用 histolab 对 whole slide image 做 tissue detection 和 tile extraction"; Grade = "M"; TaskType = "coding"; ExpectedPack = "science-medical-imaging"; ExpectedSkill = "histolab" },
    [pscustomobject]@{ Name = "medical omero microscopy"; Prompt = "用 OMERO 读取 microscopy image server 里的 ROI annotations"; Grade = "M"; TaskType = "coding"; ExpectedPack = "science-medical-imaging"; ExpectedSkill = "omero-integration" },
    ...
    [pscustomobject]@{ Name = "finance data commons"; Prompt = "用 Data Commons 查询 public statistical data 和人口经济指标"; Grade = "M"; TaskType = "research"; ExpectedPack = "finance-edgar-macro"; ExpectedSkill = "datacommons-client" },
