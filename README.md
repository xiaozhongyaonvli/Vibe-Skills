...
- 配置文件：`config/llm-acceleration-policy.json`
  - 默认 `mode=shadow`：只输出 `llm_acceleration_advice`（不改路由、不改 selected）
  - 可切换 `mode=soft`：允许将 `pack_overlay` 提升为 `confirm_required`（只“促确认”，不替换 pack/skill）
...
- 运行时依赖：
  ...
  - API Base（可选其一）：
    ...
    - env：`OPENAI_BASE_URL` / `OPENAI_API_BASE`（不同代理命名不同；如需 OpenAI 兼容路径通常应包含 `/v1`）
