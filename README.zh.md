...
&nbsp;
*这是一种全新类型的“超级技能”，其本质上相当于一套功能完备的智能体（Agent）系统。Vibe-Skills 是一套集大成的 AI 技能套件。它将专家级的能力与上下文管理机制无缝整合于一套通用技能包之中，使任何 AI 智能体都能即刻实现功能升级——从而彻底消除了因工具碎片化及复杂的集成框架所带来的种种阻滞。*
...
| **受管运行时（Governed runtime）** | 调用 `vibe` 后，系统会遵循「澄清 → 规划 → 执行 → 验证」的完整流程，而不是盲目直接执行。当前公开可发现入口固定为 `vibe` 和 `vibe-upgrade`；`Vibe: What Do I Want?`、`Vibe: How Do We Do It?`、`Vibe: Do It` 这类阶段标签属于非公开兼容入口或宿主渲染标签，底层仍回到同一个 canonical runtime authority。 |
...
> ### 🎯 核心愿景
>
> Vibe-Skills 想要创造一种新的skills形式，将harness，优秀专家skills，上下文管理整合到一个通用智能skills包中。降低AI学习和使用的认知负担。
>
...
- `vibe-what-do-i-want`、`vibe-how-do-we-do`、`vibe-do-it` 这类阶段 ID 属于兼容/非公开入口；除非宿主按 entry-surface contract 明确渲染这些标签，否则不要把它们当稳定公开入口宣传。
